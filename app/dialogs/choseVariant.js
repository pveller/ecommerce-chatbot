    ...
    bot.dialog('/choseVariant', [
        function (session, args, next) {
            ...
            if (!item.modifiers.includes('color')) {
                ...
                builder.Prompts.confirm(session, `${item.title} only comes in one color - ${item.color[0]}. Do you like it?`, {
                    listStyle: builder.ListStyle.button
                });
            } else {
                builder.Prompts.choice(session, `Please select what color you'd like best on your ${item.title}`, item.color, {
                    listStyle: builder.ListStyle.button
                });
            }
        },
        function (session, args, next) {
            ...
            if (!item.modifiers.includes('size')) {
                ...
                builder.Prompts.confirm(session, `${item.title} only comes in one size - ${item.size[0]}. Is that ok?`, {
                    listStyle: builder.ListStyle.button
                });
            } else {
                builder.Prompts.choice(session, `Please select a size for your ${item.title}`, item.size, {
                    listStyle: builder.ListStyle.button
                });
            }
        },
